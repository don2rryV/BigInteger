...
BigInteger<T, Base>::operator BigInteger<NewT, NewBase>() {
    ...
    for (size_t i = 0; i < Size(); i++) {
        ...
        BigInteger<NewT, 10> buffer = digit * pow;
        result += buffer;
        pow *= base;
    }
        ...
        for (size_t i = 0; i < std::max(val.number.size(), len_of_this) || carry; i++) {
            ...
            if (i < val.number.size()){
                for (size_t j = 0; j < num_of_blocks; j++) {
                    ...
                    if (i < len_of_this && i < val.number.size())
                        second = (*this)[i * num_of_blocks + j];
                    else second = 0;

                    if (first + second + T(carry) >= T(Base)) {
                        num += ((first - T(Base) + second + carry) << j * num_of_bits);
                        carry = 1;
                    } else {
        ...
        if ((*this).abs() >= val.abs()) {
            ...
            for (size_t i = 0; i < val.number.size() || carry; i++) {
                ...
                for (size_t j = 0; j < num_of_blocks; j++) {
                    ...
                    T second = (i < val.number.size()) ? val[i * num_of_blocks + j] : 0;
                    num += (first < second + carry) ? (((T(Base) - carry - second) + first) << j * num_of_bits)
                           : ((first - second - carry) << j * num_of_bits);
                    //num += ((T(Base) - carry - second + first) % T(Base)) << (j * num_of_bits);
...
template<typename T, size_t Base>
BigInteger<T, Base> BigInteger<T, Base>::operator*(const BigInteger &val) {
    vector<T> res(Size() + val.Size());
    vector<T> first(Size());
    vector<T> second(val.Size());
    ...
    for (int i = 0; i < (int) first.size(); i++) {
        ...
        for (int j = 0; j < (int) second.size() || carry; j++) {
            long long cur = (long long) res[i + j] +
                            ((long long) first[i] * 
							 (long long) (j < (int) second.size() ? second[j] : 0) 
							 + carry);
            res[i + j] = (T) (cur % Base);
            carry = (T) (cur / Base);
        }
    ...
    BigInteger<T, Base> ans(buf, sign == val.sign ? 1 : -1);
    ans.clear();
    return ans;
}
...
template <typename T, size_t Base>
BigInteger<T, Base> BigInteger<T, Base>::operator*(const BigInteger &val) const{
	vector <T> a(this->number.begin(), this-> number.end());
	vector <T> b(val.number.begin(), val.number.end());
    while (a.size() < b.size())
        a.push_back(0);
    while (b.size() < a.size())
        b.push_back(0);
	...
	vector <T> c = karatsubaMultiply(a, b);
	BigInteger res;
    res.sign = sign * val.sign;
    for (int i = 0, carry = 0; i < c.size(); i++) {
        long long cur = c[i] + carry;
        res.number.push_back((int) (cur % Base));
        carry = (int) (cur / Base);
    }
    res.clear();

	BigInteger<T, Base> res(c, sign == val.sign ? 1 : -1);
	res.clear();
    return res;
}*/
